...
window.addEventListener('load', function () {
    ...
    function getNameVal() { // change
        nameInput.addEventListener('input', function () {

            let val = nameInput.value.toLowerCase().trim();
            state.name = val

            const filteredForText = mainFilter()
            renderPerson(filteredForText)
        })
    }
    getNameVal()
    ...
    // function nameFilter() {
    //     return peopleArr.filter(item => item.name.toLowerCase().includes(state.name))
    // }
    ...
    function mainFilter() {
        const filtered = peopleArr.filter(item => {
            ...
            const setName = item.name.toLowerCase().includes(state.name)
